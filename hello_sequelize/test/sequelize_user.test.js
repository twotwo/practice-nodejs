...
describe('基于 Sequelize 实现的 各种表对象操作', () => {
  ...
  beforeAll((done) => {
    debug('init user_dao...');
    user_dao = sequelize.import('user', require('../models/user'));
  ...
  describe('DAO增删改查', () => {
    ...
    test('#sequelize.query 1',  (done) => {
      ...
      sequelize.query('SELECT id, username, email FROM t_project_user WHERE username = :username',
        { raw: true, replacements: { username: '李四' } })
      .then(myTableRows => {
        // debug('myTableRows = %O', myTableRows);
        done();
        expect(myTableRows[0][0]).toEqual({ id: 2, username: '李四', email: '李四@m.com' });
      }).catch(err => {
